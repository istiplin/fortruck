...
class AccountTest extends \Codeception\Test\Unit
{
    ...
    public function testRegisterUser()
    {
        ...
        $user = $this->registerUser($attr);  
                
        //убеждаемся, что роль изменена
        expect('role is mail_confirmed',$user->role_id)->equals(Role::getIdByAlias('customer'));
    ...
    {
        //отправляем форму регистрации без ошибок, для подтверждения почты
        $reg = $this->initReg($attr);
        $reg->sendMailConfirmMessage(['anyArray']);
        $user = $reg->user;
